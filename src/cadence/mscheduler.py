...
class Meetings:
    def __init__(self, name_df, name_list=None, meeting_day=2):
        ...
        if name_list is None:
            self.name_list = list(self.name_df.get(self.col_dict.get('name_col')).unique())
    ...
    def skip_date(self, cal_df, skip_date, skip_comment, skip_name='skipped'):
        ...
        if len(cal_df_skip) > 0:
            ...
            cal_df_skip.loc[cal_df_skip.get(date_col) == skip_date, name_col] = skip_name
    ...
    def create_timeboard(self, start_date, end_date, start_name):
        us_ma_holidays = holidays.country_holidays('US', subdiv='MA')
        # Rotate the speakers so that the start_name comes first
        nlist = cyclic_permutate(lst_in=self.name_list, name=start_name)
